<?php
    include("../dbsystem/config.php");
        ...
        $uploads_dir = "admin/dashboard/photo/";
        // $filename = $_FILES['photo']['name'];
        if (is_uploaded_file($_FILES['photo']['tmp_name']))
        {
            // move_uploaded_file($_FILES['photo']['tmp_name'], $uploads_dir.$filename);
...
<!DOCTYPE html>
    <html lang='en'>
                                ...
                                <div class='blog-author'>
                                    <div class='row'>
                                        ...
                                        <div class='blog-post-image'>
                                            <img src='photo/". $filename."' class='text-center blog-image' width='100%'>
                                        </div>
        ...
        $create_file_name = fopen('../../../Daily_News_&_Updates/'. $title . '.html', 'w') or die('You are not allowed');

        echo fwrite($create_file_name, $template);
        fclose('$page_name');


        $sql = "INSERT INTO `Daily_news_&_Updates`(name, remark, image, created_date)
        VALUES('$name', '$message', '$filename', now())";
